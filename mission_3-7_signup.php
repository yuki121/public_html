...
if(isset($_POST['signup'])){
	...
	if(!empty($_POST['name']) && !empty($_POST['password']) && !empty($_POST['password2']) && $_POST['password'] === $_POST['password2']){
		...
		$dsn = sprintf('mysql: host=%s; dbname=%s; charset=utf8', $db['host'], $db['dbname']);
			...
			$stmt = $pdo->prepare("INSERT INTO userdata7896 (name, password) VALUES (?, ?)");
			$stmt->execute(array($name, crypt($password)));
			$id = $pdo->lastinsertid();
			$signupMessage = '新規ユーザー登録が完了しました。登録情報は ID:'.$id.' パスワード:'.$password.' になります。';
		}
...
<br />
<input type = "submit" value = "ログインフォームに戻る" />
<br />
<br />
</fieldset>
</form>
