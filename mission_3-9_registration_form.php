...
if(empty($_GET)){
	...
	//メール入力判定
	if($urltoken == ''){
		$errors['urltoken'] = "もう一度登録し直してください。";
	}
		...
		try{
			...
			if($row_count ==1){
				...
				$errors['urltoken_timeover'] = "このURLはご利用できません。有効期限が過ぎた等の問題があります。もう一度登録し直してください。";
			}
...
<form action = "mission_3-9_registration_check.php" method = "post">
<p>メールアドレス<br /><?=htmlspecialchars($mail, ENT_QUOTES, 'UTF-8')?></p>
<p>アカウント名<br /><input type = "text" name = "account"></p>
